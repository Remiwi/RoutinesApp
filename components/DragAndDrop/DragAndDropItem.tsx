...
export default function DragAndDropItem({
  ...
  const updateStyleCoords = () => {
    const styleCoords = {
      ...
      y:
        touchScreenPos.current.y +
        (dragCtx.scrollDepth.current - dragCtx.touchHeightStart.current),
    };
    Animated.event(
      [
        {
  ...
  const remakePanResponder = () =>
    PanResponder.create({
      ...
      onPanResponderGrant: () => {
        ...
        draggingAnimationFrame.current =
          requestAnimationFrame(updateStyleCoords);
        if (onDragStarted !== undefined) onDragStarted(itemIndex);
      },
      ...
      onPanResponderEnd: () => {
        ...
        onDragFinishing?.(
          dragCtx.startIndex.current!,
          dragCtx.currentIndex.current!
        );
        ...
        Animated.timing(bubbleStylePos, {
          toValue: {
            x: 0,
            y:
              dragCtx.draggedItemHeight.current *
              (dragCtx.currentIndex.current! - itemIndex),
          },
          duration: dragCtx.droppingAnimationDuration,
          useNativeDriver: true,
  ...
  useEffect(() => {
    ...
    const evasionListenerdId = dragCtx.indexBroadcast.addListener(
      (startIndex: number, prevIndex: number, currentIndex: number) => {
        if (startIndex === itemIndex) {
          // This bubble is being dragged, so no need to evade
          return;
        }
        ...
        const BubbleUp = () =>
          Animated.timing(bubbleStylePos, {
            toValue: { x: 0, y: -dragCtx.draggedItemHeight.current },
            duration: dragCtx.evasionAnimationDuration,
            useNativeDriver: true,
          }).start();
        const BubbleReturn = () =>
          Animated.timing(bubbleStylePos, {
        ...
        if (
          ...
          // It started above me, but is now at/below me, and it wasn't there before
          ...
          // It started below me, but is now at/above me, and it wasn't there before
          ...
          // It started above me, and is now above me, and it wasn't there before
          movement = "returning";
        else if (
          startIndex > itemIndex &&
          currentIndex > itemIndex &&
          ...
          // It started below me, and is now below me, and it wasn't there before
          movement = "returning";

        if (movement === "avoiding_up") BubbleUp();
        else if (movement === "avoiding_down") BubbleDown();
        else if (movement === "returning") BubbleReturn();
      }
    );

    // Similarly, the listener for the index broadcaster will be out of date
    const indexBroadcasterListenerId = bubbleStylePos.y.addListener(
      ({ value }) => {
        ...
        const idx = Math.max(
          0,
          Math.min(dragCtx.numItems.current - 1, itemIndex + rel_idx)
        );
  ...
  }, [itemIndex, onDragStarted, onDragFinishing, onDragFinished]);
        ...
        style={[
          ...
          dragStylesEnabled ? contentContainerStyleSelected : {},
          {
            transform: [
              ...
              { translateY: bubbleStylePos.y },
            ],
          },
        ]}
        onLayout={(event) =>
          (dragCtx.draggedItemHeight.current =
            event.nativeEvent.layout.height + dragCtx.itemGap)
        }
      >
        {children}
      </Animated.View>
